...
public class FogOfWarVisual : MonoBehaviour
{
    ...
    public void ShowGridPosition(List<Vector2Int> revealedGridPosition)
    {
        ...
        foreach (GridPosition gridPosition in list)
        {
            ...
            Instantiate(exposed, LevelGrid.Instance.GetWorldPosition(gridPosition), Quaternion.identity);

            Debug.Log("gridSystemVisualSingleArray exists: " + gridSystemVisualSingleArray != null);
            gridSystemVisualSingleArray
            [gridPosition.x, 
            gridPosition.z]
                .Show(
                    GetGridVisualTypeMaterial(
                        gridVisualType));
